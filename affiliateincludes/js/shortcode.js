...
function affReBuildChart(chart, ticks) {
    ...
    jQuery("#affdashgraph").on("plothover", function(event, pos, item) {
        if (item) {
...
function affReBuildVisits(chart, ticks) {
    ...
    var previousPoint = null;
    jQuery("#affvisitgraph").on("plothover", function(event, pos, item) {
        if (item) {
            if (previousPoint != item.datapoint) {
                previousPoint = item.datapoint;
...
function affReportReady() {
    ...
    jQuery('#editaffsettingslink').on("click", affToggleView);
