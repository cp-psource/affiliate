...
Description: Belohne Benutzer für das Einbringen neuer Abonnements über das PSOURCE PS Mitgliedschaften-Plugin
Author URI: https://n3rds.work/piestingtal-source-project/ps-mitglieder-plugin/
...
class Affiliate_Membership2_Integration {
	 ...
	 * to save the Partnerprogramm reward options.
	 */
	...
	public function init( $api ) {
		...
		if ( MS_Plugin::is_network_wide() ) {
			...
			if ( ! is_plugin_active_for_network( $affiliate_plugin )
				...
				$this->ms_lib->ui->admin_message(
					__( 'PS Mitgliedschaften verwendet einen netzwerkweiten Schutz.<br>Bitte aktiviere das Partnerprogramm-Plugin im Netzwerk, um Probleme mit der PS Mitgliedschaften-Integration für Partner zu vermeiden.', 'affiliate' ),
					'err'
				);
			}
		}
		...
		// Display Partnerprogramm settings on the Membership payment options page.
		add_action(
		...
		// Save Partnerprogramm settings via Ajax request.
		add_action(
	...
	public function payment_processed( $invoice, $subscription ) {
		...
		do_action(
			...
			self::AREA_KEY,
			$user_id,
			__( 'PS Mitgliedschaften', 'affiliate' ),
			$meta
		);
	...
	public function form( $view, $membership ) {
		...
		if ( $membership->is_free ) {
			$label = __( 'Belohnung für Abonnement', 'affiliate' );
		} else {
			$label = __( 'Belohnung bei der ersten Zahlung', 'affiliate' );
		}
		...
		$fields['type'] = array(
			...
			'field_options' => array(
				'inv' => __( 'Prozent (Zahlung)', 'affiliate' ) . '*',
				'mem' => __( 'Prozent (Preis)', 'affiliate' ) . '**',
		...
		<div class="aff-payment-options">
			...
			<div class="aff-title">
				<strong><?php _e( 'Partnerprogramm Einstellungen', 'affiliate' ); ?></strong>
			</div>
			<div class="aff-inside">
				...
				?>
				<p>
				* <?php _e( 'Die Belohnung basiert auf dem tatsächlich gezahlten Betrag, der alle Rabatte (aber keine Steuern) enthält.', 'affiliate' ); ?><br />
				** <?php _e( 'Die Belohnung basiert auf der aktuellen Einstellung des Mitgliedschaftspreises.', 'affiliate' ); ?>
				</p>
				<p>
				<?php
				printf(
					__( 'Tipp: Es erfolgt keine Währungsumrechnung. Alle Prämien werden in %s ausgezahlt, unabhängig von der von PS Mitgliedschaften oder der Rechnung verwendeten Währung.', 'affiliate' ),
	...
	/**
	 * Ajax handler that saves a single Partnerprogramm reward option from the
