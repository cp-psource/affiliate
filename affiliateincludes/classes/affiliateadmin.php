...
class affiliateadmin {
	...
	function __construct() {
		...
		// Partnerprogramm blog and user reporting
	...
	function ajax__aff_getstats() {
		...
		$headings = aff_get_option( 'affiliateheadings', array(
			__( 'Einzigartige Klicks', 'affiliate' ),
			__( 'Registrierungen', 'affiliate' ),
			__( 'Bezahlte Mitglieder', 'affiliate' )
		) );
	...
	function add_menu_items() {
		...
		add_submenu_page( 'affiliatesadmin', __( 'Affiliates verwalten', 'affiliate' ), __( 'Affiliates verwalten', 'affiliate' ), $capabilty, 'affiliatesadminmanage', array(
			&$this,
			'handle_affiliates_panel'
		) );
		add_submenu_page( 'affiliatesadmin', __( 'Einstellungen', 'affiliate' ), __( 'Einstellungen', 'affiliate' ), $capabilty, 'affiliatesadminsettings', array(
			&$this,
			'handle_affiliates_panel'
		) );
		add_submenu_page( 'affiliatesadmin', __( 'Erweiterungen', 'affiliate' ), __( 'Erweiterungen', 'affiliate' ), $capabilty, 'affiliatesadminaddons', array(
			&$this,
			'handle_affiliates_panel'
		) );

		if ( isset( $submenu['affiliatesadmin'] ) ) {
			$submenu['affiliatesadmin'][0][0] = __( 'Partnerprogramm Berichte', 'affiliate' );
		}

		add_submenu_page( 'users.php', __( 'Partnerprogramm Ergebnisbericht', 'affiliate' ), __( 'Partnerprogramm-Empfehlungen', 'affiliate' ), 'read', "affiliateearnings", array(
			&$this,
		...
		if ( get_user_meta( $user_ID, 'enable_affiliate', true ) == 'yes' ) {
			if ( aff_get_option( 'affiliateenablebanners', 'no' ) == 'yes' ) {
				add_submenu_page( 'users.php', __( 'Partnerprogramm Banner', 'affiliate' ), __( 'Partnerprogramm Banner', 'affiliate' ), 'read', "affiliatebanners", array(
					&$this,
	...
	function add_profile_report_page() {
		...
		$headings = array_merge( $headings, array(
			__( 'Belastungen', 'affiliate' ),
			__( 'Gutschriften', 'affiliate' ),
			__( 'Auszahlungen', 'affiliate' )
		) );
		...
		if ( isset( $_POST['action'] ) && addslashes( $_POST['action'] ) == 'update' ) {
			...
			if ( ! empty( $_POST['affiliate_referrer'] ) ) {
				...
				if ( $this->is_duplicate_url( $url, $user_ID ) ) {
					$error  = 'yes';
					$chkmsg = __( 'Diese URL wird bereits verwendet.', 'affiliate' );
				} else {
					...
					if ( defined( 'AFFILIATE_VALIDATE_REFERRER_URLS' ) && AFFILIATE_VALIDATE_REFERRER_URLS == 'yes' ) {
						...
						if ( $this->validate_url_for_file( trailingslashit( $url ), $filename ) ) {
							...
							$chkmsg = __( 'Bestätigt', 'affiliate' );
						} else {
							$error  = 'yes';
							$chkmsg = __( 'Nicht validiert', 'affiliate' );
						}
					}
				}
			} else {
		...
		echo "<h2>" . __( 'Partnerprogramm Empfehlungsbericht', 'affiliate' ) . "</h2>";
		...
		if ( get_user_meta( $user_ID, 'enable_affiliate', true ) == 'yes' ) {
			echo "<strong>" . __( 'Hallo, vielen Dank, dass Du uns unterstützt.</strong> Um Deine Partnereinstellungen anzuzeigen oder zu ändern, klicke auf den Bearbeitungslink.', 'affiliate' ) . "</strong><a href='#view' id='editaffsettingslink' style='float:right; font-size: 8pt;'>" . __( 'Bearbeiten', 'affiliate' ) . "</a>";
			...
			$settingstextdefault = __( "<p>Wir lieben es, wenn Leute über uns sprechen, und noch mehr, wenn sie uns ihren Freunden empfehlen.</p><p>Als Dankeschön möchten wir etwas zurückgeben, weshalb wir dieses Partnerprogramm eingerichtet haben.</p><p>Um zu beginnen, aktiviere einfach die Links für Dein Konto und gib unten Deine PayPal-eMail-Adresse ein. Weitere Informationen zu unserem Partnerprogramm findest Du auf unserer Hauptseite.</p>", 'affiliate' );
			...
			<table class="form-table">
				<tr style='background: transparent;'>
					<th><label for="enable_affiliate"><?php _e( 'Partnerprogramm-Links aktivieren', 'affiliate' ); ?></label></th>
					<td>
						<select name='enable_affiliate'>
							<option
								value='yes' <?php if ( get_user_meta( $user_ID, 'enable_affiliate', true ) == 'yes' ) {
								echo "selected = 'selected'";
							} ?>><?php _e( 'Ja Bitte', 'affiliate' ); ?></option>
							<option
								value='no' <?php if ( get_user_meta( $user_ID, 'enable_affiliate', true ) != 'yes' ) {
								echo "selected = 'selected'";
							} ?>><?php _e( 'Nein Danke', 'affiliate' ); ?></option>
						</select>
					</td>
				</tr>

				<tr style='background: transparent;'>
					<th><label for="affiliate_paypal"><?php _e( 'PayPal Email Addresse', 'affiliate' ); ?></label></th>
					<td>
			...
			if ( get_user_meta( $user_ID, 'enable_affiliate', true ) == 'yes' ) {
				...
				?>
				<p><?php _e( '<h3>Partnerprogramm Details</h3>', 'affiliate' ) ?></p>
				<p><?php _e( 'Damit wir Deine Empfehlungen verfolgen können, solltest Du die folgende URL verwenden, um auf unsere Webseite zu verlinken:', 'affiliate' ); ?></p>
				...
				if ( aff_get_option( 'affiliateenablebanners', 'no' ) == 'yes' ) {
					?>
					<p><?php echo sprintf( __( 'Wenn Du lieber ein Banner oder eine Schaltfläche verwenden möchtest, haben wir <a href="%s">hier</a> eine große Auswahl an Größen.', 'affiliate' ), "profile.php?page=affiliatebanners" ); ?></p>
				<?php } ?>
				...
				if ( defined( 'AFFILIATE_CHECKALL' ) && AFFILIATE_CHECKALL == 'yes' ) { ?>
					...
					<h3><?php _e( 'Partnerprogramm Erweiterte Einstellungen', 'affiliate' ) ?></h3>

					<?php
					$advsettingstextdefault = __( "<p>Es gibt Zeiten, in denen Du Deinen Partnerprogramm-Link lieber ausblenden möchtest oder einfach nicht an die Partnerprogramm-Referenz erinnern musst, um sie am Ende unserer URL anzubringen.</p><p>Wenn dies der Fall ist, kannst Du die Haupt-URL der Seite eingeben, an die Du Anfragen von unten sendest, und wir werden die kniffligen Teile für Dich übernehmen um Deine Partnerprogramm zu berücksichtigen.</p>", 'affiliate' );
					...
					<table class="form-table">
						<tr style='background: transparent;'>
							<th><label for="affiliate_referrer"><?php _e( 'Deine URL', 'affiliate' ); ?></label></th>
							<td>
								...
								if ( defined( 'AFFILIATE_VALIDATE_REFERRER_URLS' ) && AFFILIATE_VALIDATE_REFERRER_URLS == 'yes' ) {
									if ( empty( $referrer ) || ( ! empty( $validreferrer ) && $validreferrer == 'yes' ) ) {
										...
										// Output message
										echo "<br/>";
										_e( 'Du musst diese URL überprüfen, indem Du eine Datei mit dem folgenden Namen in das Stammverzeichnis der oben genannten Seite hochlädst : ', 'affiliate' );
										echo "<br/>";
										echo __( 'Dateiname : ', 'affiliate' ) . $filename;
										echo " <a href='" . $schema . trailingslashit( $referrer ) . $filename . "' target=_blank>" . __( '[Klicke hier, um zu überprüfen, ob die Datei vorhanden ist]', 'affiliate' ) . "</a>";
										echo '<br/><input type="submit" name="Submit" class="button" value="' . __( 'Bestätigen', 'affiliate' ) . '" />';
									}
								}
			...
			echo '<p class="submit">';
			echo '<input type="submit" class="button-primary" name="Submit" value="' . __( 'Einstellungen speichern', 'affiliate' ) . '" /></p>';
			...
			echo "<strong>" . __( 'Hallo, warum nicht ein Partner werden?', 'affiliate' ) . "</strong><br/>";
		...
		if ( get_user_meta( $user_ID, 'enable_affiliate', true ) == 'yes' ) {
			...
			if ( ( isset( $_GET['subpage'] ) ) && ( $_GET['subpage'] == "details" ) ) {
				echo '<br /><a href="' . esc_url( add_query_arg( 'subpage', 'summary' ) ) . '">' . __( '&larr; Zurück zur Zusammenfassung des Partnerzeitraums', 'affiliate' ) . '</a>';
			}
			...
			echo "<tr>";
			echo "<th scope='col'>";
			echo __( 'Top Überweiser für ', 'affiliate' ) . date( "M Y" );
			echo "</th>";
			echo "<th scope='col' style='width: 3em;'>";
			echo __( 'Besuche', 'affiliate' );
			echo "</th>";
			echo "</tr>";
			echo "</thead>";
			...
			if ( ! empty( $rows ) ) {
				...
				echo __( '<tr><td colspan="2">Du hast diesen Monat keine empfohlenen Besuche.</td></tr>', 'affiliate' );
			}
			...
			echo "<tr>";
			echo "<th scope='col'>";
			echo __( 'Top-Überweiser in den letzten 18 Monaten', 'affiliate' );
			echo "</th>";
			echo "<th scope='col' style='width: 3em;'>";
			echo __( 'Besuche', 'affiliate' );
			echo "</th>";
			...
			if ( ! empty( $rows ) ) {
				...
				echo __( '<tr><td colspan="2">Du hast insgesamt keine überwiesenen Besuche.</td></tr>', 'affiliate' );
			}
	...
	function add_profile_banner_page() {
		...
		?>
		<div class='wrap'>
			<h2>Partnerprogramm Banners</h2>

			<p><?php _e( "Du möchtest also etwas Aufregenderes als einen einfachen Textlink?", 'affiliate' ); ?></p>
			<p><?php _e( "Keine Sorge, wir haben jede Menge Banner und Buttons. Um sie zu verwenden, kopiere einfach den HTML-Code und füge ihn unter der Grafik ein, die Du verwenden möchtest.", 'affiliate' ); ?></p>
					...
					if ( isset( $_POST['allaction_exportpayments'] ) ) {
						...
						if ( ! empty( $_POST['allpayments'] ) ) {
							foreach ( $_POST['allpayments'] as $affiliate ) {
								...
								$id       = "AFF_PAYMENT";
								$notes    = __( "Partnerprogramm-Zahlung für", "affiliate" );
								...
								if ( count( $affdetails ) == 2 ) {
									...
									$notes = __( "Partnerprogramm-Zahlung für ", "affiliate" ) . $name;
	...
	function handle_affiliate_settings_panel() {
		...
		if ( isset( $_GET['action'] ) && addslashes( $_GET['action'] ) == 'updateaffiliateoptions' ) {
			...
			echo '<div id="message" class="updated fade"><p>' . __( 'Partnerprogramm-Einstellungen gespeichert.', 'affiliate' ) . '</p></div>';
		}
		...
		echo '<p class="submit">';
		echo '<input type="submit" name="Submit" value="' . __( 'Eistellungen speichern', 'affiliate' ) . '" class="button-primary" /></p>';
	...
	function handle_affiliate_users_panel() {
		...
		if ( isset( $_REQUEST['id'] ) ) {
				...
				switch ( addslashes( $_POST['action'] ) ) {
					...
					case 'userdebit':
						...
						if ( $queryresult ) {
							...
							echo '<div id="message" class="updated fade"><p>' . __( 'Die Lastschrift wurde korrekt zugewiesen.', 'affiliate' ) . '</p></div>';
						}
						break;
					case 'usercredit':
						...
						if ( $queryresult ) {
							...
							echo '<div id="message" class="updated fade"><p>' . __( 'Das Guthaben wurde korrekt zugewiesen.', 'affiliate' ) . '</p></div>';
						}
						break;
					case 'userpayment':
						...
						if ( $queryresult ) {
							...
							echo '<div id="message" class="updated fade"><p>' . __( 'Die Zahlung wurde korrekt zugewiesen.', 'affiliate' ) . '</p></div>';
						}
						break;
					case 'findusers':
			...
			echo "<strong>" . __( 'Details für den Benutzer: ', 'affiliate' ) . $user->user_login . " ( " . get_user_meta( $user_id, 'affiliate_paypal', true ) . " )" . "</strong>";
			// Get the affiliate website listing
			$referrer = get_user_meta( $user_id, 'affiliate_referrer', true );
                        $schema = is_ssl() ? 'https://' : 'http://';
			if ( ! empty( $referrer ) ) {
                                echo " " . __( 'verbunden mit ', 'affiliate' ) . "<a href='{$schema}{$referrer}'>" . $referrer . "</a>";
			}


			if ( ( isset( $_GET['subpage'] ) ) && ( $_GET['subpage'] == "details" ) ) {
				echo '<br /><a href="' . esc_url( add_query_arg( 'subpage', 'summary' ) ) . '">' . __( '&larr; Zurück zur Zusammenfassung des Partnerprogrammzeitraums', 'affiliate' ) . '</a>';
			}
			...
			if ( aff_get_option( 'affiliateenableapproval', 'no' ) == 'yes' ) {
				...
				echo "<tr>";
				echo "<th scope='col'>";
				echo __( 'Benutzerkonto genehmigen', 'affiliate' );
				echo "</th>";
				echo "<th scope='col' style='width: 3em;'>";
				echo '&nbsp;';
				echo "</th>";
				...
				echo "<tr class='' style=''>";
				echo "<td style='padding: 5px;'>";
				echo __( 'Benutzer ist ', 'affiliate' );
				echo '<select name="userapproved" id="userapproved">';
				echo '<option value="no" ' . selected( $app, 'no', false ) . '>' . __( 'nicht bestätigt', 'affiliate' ) . "</option>";
				echo '<option value="yes" ' . selected( $app, 'yes', false ) . '>' . __( 'bestätigt', 'affiliate' ) . "</option>";
				echo '</select>&nbsp;';
				echo __( ' um Partnerprogramm-Zahlungen zu erhalten.', 'affiliate' );
				echo "</td>";
				echo "<td style='width: 3em; padding: 5px; text-align: right;'>";
				echo "<input type='submit' name='approveaccount' value='" . __( 'Aktualisieren', 'affiliate' ) . "' class='button-primary' />";
				echo "</td>";
				echo "</tr>";
			...
			echo "<tr>";
			echo "<th scope='col'>";
			echo __( 'Guthaben Benutzerkonto', 'affiliate' );
			echo "</th>";
			echo "<th scope='col' style='width: 3em;'>";
			echo '&nbsp;';
			echo "</th>";
			...
			echo "<tr class='' style=''>";
			echo "<td style='padding: 5px;'>";
			echo '<label for="creditperiod">' . __( 'Zeitraum : ', 'affiliate' ) . '</label>';
			echo '<select name="creditperiod" id="creditperiod">';
			echo $period_options;
			echo '</select>&nbsp;';

			echo '<label for="creditvalue">' . __( 'Wert : ', 'affiliate' ) . '</label>';
			echo '<input type="text" id="creditvalue" name="creditvalue" value="" style="width: 10%;"/>&nbsp;';

			echo '<label for="creditnote">' . __( 'Hinweis : ', 'affiliate' ) . '</label>';
			echo '<input type="text" id="creditnote" name="creditnote" value="" style="width: 45%;" placeholder="' . __( 'Gutschrift für...', 'affiliate' ) . '"/>';

			echo "</td>";
			echo "<td style='width: 3em; padding: 5px; text-align: right;'>";
			echo "<input type='submit' name='creditaccount' value='" . __( 'Guthaben hinzufügen', 'affiliate' ) . "' class='button-primary' />";
			echo "</td>";
			echo "</tr>";
			...
			echo "<tr>";
			echo "<th scope='col'>";
			echo __( 'Benutzerkonto belasten', 'affiliate' );
			echo "</th>";
			echo "<th scope='col' style='width: 3em;'>";
			echo '&nbsp;';
			echo "</th>";
			...
			echo "<tr class='' style=''>";
			echo "<td style='padding: 5px;'>";
			echo '<label for="debitperiod">' . __( 'Zeitraum : ', 'affiliate' ) . '</label>';
			echo '<select name="debitperiod" id="debitperiod">';
			echo $period_options;
			echo '</select>&nbsp;';
			echo '<label for="debitvalue">' . __( 'Wert : ', 'affiliate' ) . '</label>';
			echo '<input type="text" id="debitvalue" name="debitvalue" value="" style="width: 10%;"/>';
			echo '  <label for="debitnote">' . __( 'Hinweis : ', 'affiliate' ) . '</label>';
			echo '<input type="text" id="debitnote" name="debitnote" value="" style="width: 45%;" placeholder="' . __( 'Lastschrift für ...', 'affiliate' ) . '"/>';
			echo "</td>";
			echo "<td style='width: 3em; padding: 5px; text-align: right;'>";
			echo "<input type='submit' name='debitaccount' value='" . __( 'Lastschrift hinzufügen', 'affiliate' ) . "' class='button-primary' />";
			echo "</td>";
			echo "</tr>";
			...
			echo "<tr>";
			echo "<th scope='col'>";
			echo __( 'Zahlung auf Konto einstellen', 'affiliate' );
			echo "</th>";
			echo "<th scope='col' style='width: 3em;'>";
			echo '&nbsp;';
			echo "</th>";
			...
			echo "<tr class='' style=''>";
			echo "<td style='padding: 5px;'>";
			echo '<label for="payperiod">' . __( 'Zeitraum : ', 'affiliate' ) . '</label>';
			echo '<select name="payperiod" id="payperiod">';
			echo $period_options;
			echo '</select>&nbsp;';

			echo '<label for="payvalue">' . __( 'Wert : ', 'affiliate' ) . '</label>';
			echo '<input type="text" id="payvalue" name="payvalue" value="" style="width: 10%;" />&nbsp;';

			echo '<label for="paynote">' . __( 'Hinweis : ', 'affiliate' ) . '</label>';
			echo '<input type="text" id="paynote" name="paynote" value="" style="width: 45%;" placeholder="' . __( 'Zahlung für...', 'affiliate' ) . '"/>';

			echo "</td>";
			echo "<td style='width: 3em; padding: 5px; text-align: right;'>";
			echo "<input type='submit' name='payaccount' value='" . __( 'Zahlung hinzufügen', 'affiliate' ) . "' class='button-primary' />";
			echo "</td>";
			echo "</tr>";
			...
			<form action="" method="get">
				...
				$aff_list_table->search_box( __( 'Benutzer suchen' ), 'user' );
				?>
	...
	function handle_affiliates_panel() {
		...
		echo "<h2>" . __( 'Partnerprogramm-Systemadministration', 'affiliate' ) . "</h2>";
		...
		if ( ! empty( $page ) && $page != 'affiliatesadmin' ) {
					...
					case 'allaffiliates':    // Bulk operations
						if ( isset( $_POST['allaction_markaspaid'] ) ) {
							...
							if ( ! empty( $_POST['allpayments'] ) ) {
								...
								echo '<div id="message" class="updated fade"><p>' . __( 'Zahlungen wurden korrekt zugeordnet.', 'affiliate' ) . '</p></div>';
							}
								...
								if ( $record ) {
									...
									if ( $balance > 0 ) {
										...
										if ( $affected ) {
											...
											$note = __( 'Zahlung an den Partner', 'affiliate' );
			...
			$headings = array_merge( $headings, array(
				__( 'Guthaben', 'affiliate' ),
				__( 'Belastungen', 'affiliate' ),
				__( 'Zahlungen', 'affiliate' ),
				__( 'Billanz', 'affiliate' )
			) );
			...
			echo __( 'Bericht anzeigen für', 'affiliate' ) . '&nbsp;';
			...
			echo '</select>&nbsp;';
			echo '<input type="submit" value="' . __( 'Aktualisierung', 'affiliate' ) . '" name="allaction_refresh" class="button-secondary" />';
			...
			echo '<input type="submit" value="' . __( 'Zahlungen exportieren', 'affiliate' ) . '" name="allaction_exportpayments" class="button-secondary delete" />&nbsp;&nbsp;';
			echo '<input type="submit" value="' . __( 'Guthaben bezahlen', 'affiliate' ) . '" name="allaction_markaspaid" class="button-secondary" />';
			...
			echo '<th scope="col">';
			echo __( 'Benutzername', 'affiliate' );
			echo '</th>';

			if ( aff_get_option( 'affiliateenableapproval', 'no' ) == 'yes' ) {
				echo '<th scope="col">';
				echo __( 'Genehmigt', 'affiliate' );
				echo '</th>';
			}
			...
			if ( $results ) {
				foreach ( $results as $result ) {
					...
					$actions = array();
					if ( $this->approved_affiliate( $result->user_id ) ) {
						$actions[] = "<a href='?page=$page&amp;action=makepayment&amp;id=" . $result->user_id . "-" . $result->period . "&amp;reportperiod=" . $reportperiod . "' class='edit'>" . __( 'Zahlungsbillanz', 'affiliate' ) . "</a>";
					}
					$actions[] = "<a href='?page=affiliatesadminmanage&amp;subpage=summary&amp;id=" . $result->user_id . "' class='edit'>" . __( 'Partner verwalten', 'affiliate' ) . "</a>";
...
*/
					...
					if ( aff_get_option( 'affiliateenableapproval', 'no' ) == 'yes' ) {
						...
						if ( $this->approved_affiliate( $result->user_id ) ) {
							echo __( 'Ja', 'affiliate' );
						} else {
							echo __( 'Nein', 'affiliate' );
						}
						echo '</td>';
					}
				...
				echo __( 'Für den ausgewählten Monat liegen keine Ergebnisse vor.', 'affiliate' );
				echo '</td>';
				echo '</tr>';
			}
	...
	function add_user_affiliate_column( $columns ) {
		$columns['referred'] = __( 'Empfohlen von', 'affiliate' );
	...
	function handle_addons_panel() {
		...
		$messages    = array();
		$messages[1] = __( 'Erweiterung aktualisiert.', 'affiliate' );
		$messages[2] = __( 'Erweiterung nicht aktualisiert.', 'affiliate' );

		$messages[3] = __( 'Erweiterung aktiviert.', 'affiliate' );
		$messages[4] = __( 'Erweiterung nicht aktiviert.', 'affiliate' );

		$messages[5] = __( 'Erweiterung deaktiviert.', 'affiliate' );
		$messages[6] = __( 'Erweiterung nicht deaktiviert.', 'affiliate' );

		$messages[7] = __( 'Erweiterung-Aktivierung umgeschaltet.', 'affiliate' );
		...
		<form method="get"
			...
			<div class="tablenav">
				...
				<div class="alignleft actions">
					...
					<input type="submit" class="button-secondary action" id="doaction" name="doaction"
					       value="<?php _e( 'Anwenden', 'affiliate' ); ?>">
			...
			$columns = array(
				"name"   => __( 'Erweiterung Name', 'affiliate' ),
				"active" => __( 'Erweiterung Status', 'affiliate' )
			);
				...
				if ( ! empty( $plugins ) ) {
					...
					foreach ( $plugins as $key => $plugin ) {
						...
						<tr valign="middle" class="alternate" id="plugin-<?php echo $plugin; ?>">
							...
							<td class="column-name">
								...
								<?php
								if ( ! $PLUGIN_INSTALLED ) {
									//echo ' <span>'. __('Basis-Plugin nicht aktiviert/installiert', 'affiliate') .'</span>';
									echo ' -- <strong>' . __( 'Plugin nicht installiert', 'affiliate' ) . '</strong>';
								}
								...
								//	echo '<br /><strong>' .__('Network only - Requires Partnerprogramm plugin is Network Activated', 'affiliate') .'</strong>';
								//}

								if ( ( $PLUGINS_CAN_BE_ACTIVE ) && ( $PLUGIN_INSTALLED ) ) {
									$actions = array();
									if ( in_array( $plugin, $active ) ) {
										$actions['toggle'] = "<span class='edit activate'><a href='" . wp_nonce_url( "?page=" . $page . "&amp;subpage=" . $subpage . "&amp;action=deactivate&amp;addon=" . $plugin . "", 'toggle-addon-' . $plugin ) . "'>" . __( 'Deaktivieren', 'affiliate' ) . "</a></span>";
									} else {
										$actions['toggle'] = "<span class='edit deactivate'><a href='" . wp_nonce_url( "?page=" . $page . "&amp;subpage=" . $subpage . "&amp;action=activate&amp;addon=" . $plugin . "", 'toggle-addon-' . $plugin ) . "'>" . ( ( function_exists( 'is_network_admin' ) && is_network_admin() ) ? __( 'Netzwerk aktivieren', 'affiliate' ) : __( 'Aktivieren', 'affiliate' ) ) . "</a></span>";
									}
									if ( count( $actions ) ) {
										?><br/>
							...
							<td class="column-active">
								...
								if ( ( $PLUGINS_CAN_BE_ACTIVE ) && ( $PLUGIN_INSTALLED ) ) {
									if ( in_array( $plugin, $active ) ) {
										echo "<strong>" . __( 'Aktiv', 'affiliate' ) . "</strong>";
									} else {
										echo __( 'Inaktiv', 'affiliate' );
									}
								}
								?>
							</td>
					...
					<tr valign="middle" class="alternate">
						<td colspan="<?php echo $columncount; ?>"
						    scope="row"><?php _e( 'Für diese Installation wurden keine Erweiterungen gefunden.', 'affiliate' ); ?></td>
					</tr>
					<?php
				}
				?>
	...
	function add_referrer_search_link( $actions, $user_object ) {
		...
		if ( is_network_admin() ) {
			$actions['referred'] = '<a href="' . network_admin_url( 'users.php?s=referrer:' . $user_object->user_login ) . '">' . __( 'Weitergeleitet', 'affiliate' ) . '</a>';
		} else {
			$actions['referred'] = '<a href="' . admin_url( 'users.php?s=referrer:' . $user_object->user_login ) . '">' . __( 'Weitergeleitet', 'affiliate' ) . '</a>';
		}
	...
	function edit_user_profile( $user = '' ) {
		...
		?><h3><?php _e( 'Partnerprogramm-Transaktionen', 'affiliate' ); ?></h3><?php
	...
	function show_complete_records_table( $user_id, $period = false, $area = false, $area_id = false ) {
		...
		<table id="affiliate-details" class="affiliate-details">
			<tr>
				<th class="affiliate-period"><?php _e( 'Zeitraum', 'affiliate' ); ?></th>
				<th class="affiliate-type"><?php _e( 'Typ', 'affiliate' ); ?></th>
				<th class="affiliate-period"><?php _e( 'Hinweis', 'affiliate' ); ?></th>
				<th class="affiliate-period"><?php _e( 'Betrag', 'affiliate' ); ?></th>
			</tr>
			<?php
			...
			if ( ( $compete_records ) && ( ! empty( $compete_records ) ) ) {
					...
					<tr <?php echo $style; ?>>
						...
						<td class="affiliate-type"><?php
							...
							if ( ( $compete_record->affiliatearea == 'paid:psecommerce' ) && ( ! empty( $compete_record->area_id ) ) ) {
								...
								echo __( 'Empfehlungslink', 'affiliate' );
							} else {
				...
				if ( count( $compete_records ) > 1 ) {
					...
					<tr>
						...
						<td class="affiliate-note"><?php _e( 'Gesamt', 'affiliate' ); ?></td>
				...
				?>
				<tr>
				<td colspan="4"><?php _e( 'Für diesen Benutzer wurden keine Partnerprogramm-Transaktionen gefunden.', 'affiliate' ); ?></td>
				</tr><?php
			}
			?>
		</table>
		...
		for ( $n = 0; $n < 18; $n ++ ) {
			...
			if ( ( ! empty( $recent->uniques ) ) || ( ! empty( $recent->signups ) ) || ( ! empty( $recent->completes ) )
				...
				*/
				...
				echo '<a title="' . esc_attr( __( 'Partnerprogramm-Detailtransaktionen für diesen Zeitraum anzeigen', 'affiliate' ) ) . '" href="' . $period_url . '">' . date_i18n( "M", $rdate, true ) . '<br/>' . date_i18n( "Y", $rdate ) . '</a>';
	...
	function show_users_period_details_table( $user_id, $period ) {
		$columns = array(
			'date'   => __( 'Datum', 'affiliate' ),
			'type'   => __( 'Typ', 'affiliate' ),
			'note'   => __( 'Hinweis', 'affiliate' ),
			'amount' => __( 'Betrag', 'affiliate' )
		);
					...
					switch ( $key ) {
						...
						case 'type':
							...
							if ( ( $record->affiliatearea == 'paid:psecommerce' ) && ( ! empty( $record->area_id ) ) ) {
								...
								//	_e('Benutzer registrieren', 'affiliate');
								//else if ($record->affiliatearea == "signup:blog")
								//	_e('Blog registrieren', 'affiliate');
								//else
								_e( 'Registrieren', 'affiliate' );
							} else {
									...
									if ( ( $user ) && ( intval( $user->ID ) === intval( $record->area_id ) ) ) {
										...
										if ( current_user_can( 'edit_users' ) ) {
											if ( is_network_admin() ) {
												...
												echo ' <a title="' . __( 'Benutzer anzeigen', 'affiliate' ) . '" href="' . admin_url( 'user-edit.php?user_id=' . $user_ID ) . '">' . $display_name . '</a>';
											}
										} else {
											echo ' ' . $display_name;
										}
									} else {
										echo ' ' . __( 'Benutzeridentifikation', 'affiliate' ) . ' ' . $record->area_id;
									}
		...
		echo '<td colspan="' . $col_count . '">' . __( 'Periodensaldo', 'affiliate' ) . '</td>';
