...
class affiliateshortcodes {
	...
	function check_for_shortcodes( ) {
		...
		if ( ! current_theme_supports( 'affiliate_styles' ) ) {
			// Partnerprogramm shortcode tags.
			$shortcodes = array(
				'affiliatelogincheck',
	...
	function do_affiliatelogincheck_shortcode($atts, $content = null, $code = "") {
		...
		if(empty($user_ID)) {
			...
			if(empty($content)) {
				$html .= sprintf( __( 'Du bist derzeit nicht angemeldet. Bitte %s, um Deine Partnerinformationen anzuzeigen.','affiliate' ), '<a href="' . wp_login_url() . '">' . __('anmelden','affiliate') . '</a>' );
			} else {
				$html .= $content;
			}
			$html .= "</div>";
	...
	function show_clicks_chart() {
		...
		$headings = array_merge($headings, array( __('Belastungen','affiliate'), __('Gutschriften','affiliate'), __('Zahlungen','affiliate') ));
	...
	function show_visits_table() {
		...
		echo "<thead>";
			echo "<tr>";
			echo "<th scope='col'>";
			echo  __('Top Empfehler für','affiliate') .' '. date_i18n("M Y");
			echo "</th>";
	...
	function show_top_visits_table() {
		...
		echo "<thead>";
			echo "<tr>";
			echo "<th scope='col'>";
			echo  __('Top-Empfehler in den letzten 18 Monaten','affiliate');
			echo "</th>";
			echo "<th scope='col' style='width: 3em;'>";
			echo __('Besuche','affiliate');
			echo "</th>";
	...
	function show_user_details( $bannerlink = '' ) {
		...
		if(get_user_meta($user_ID, 'enable_affiliate', true) == 'yes') {
			...
			$settingstextdefault = "<p>Wir lieben es, wenn Leute über uns sprechen, und noch mehr, wenn sie uns ihren Freunden empfehlen.</p>
		<p>Als Dankeschön möchten wir etwas zurückgeben, weshalb wir dieses Partnerprogramm eingerichtet haben.</p>
		<p>Um zu beginnen, aktiviere einfach die Links für Dein Konto und gib unten Deine PayPal-E-Mail-Adresse ein. Weitere Informationen zu unserem Partnerprogramm findest Du auf unserer Hauptseite.</p>";
				...
				<tr style='background: transparent;'>
					...
					<td>
						<select name='enable_affiliate'>
							<option value='yes' <?php if(get_user_meta($user_ID, 'enable_affiliate', true) == 'yes') echo "selected = 'selected'"; ?>><?php _e('Ja Bitte', 'affiliate'); ?></option>
							<option value='no' <?php if(get_user_meta($user_ID, 'enable_affiliate', true) != 'yes') echo "selected = 'selected'"; ?>><?php _e('Nein Danke', 'affiliate'); ?></option>
						</select>
					</td>
				</tr>
			...
			if(get_user_meta($user_ID, 'enable_affiliate', true) == 'yes') {
				...
				if((defined('AFFILIATE_CHECKALL')) && (AFFILIATE_CHECKALL == 'yes')) { ?>
					...
					<h3><?php _e('Erweiterte Partnereinstellungen', 'affiliate') ?></h3>

					<?php
					$advsettingstextdefault = "<p>Es gibt Zeiten, in denen Du Deinen Partnerprogramm-Link lieber ausblenden möchtest oder einfach nicht an die Partnerprogramm-Referenz erinnern musst, um sie am Ende unserer URL anzubringen.</p>
				<p>Wenn dies der Fall ist, kannst Du die Haupt-URL der Seite eingeben, an die Du Anfragen von unten sendest, und wir werden die kniffligen Teile für Dich erledigen um Deine Partnerprogramm dennoch zu berücksichtigen.</p>";
					...
					<table class="form-table">
						<tr style='background: transparent;'>
							<th valign='top'><label for="affiliate_referrer"><?php _e('Deine URL', 'affiliate'); ?></label></th>
							<td>
				...
				<p><?php _e('Damit wir Deine Empfehlungen verfolgen können, solltest Du die folgende URL verwenden, um auf unsere Website zu verlinken:', 'affiliate'); ?></p>
				...
				<?php
					if(defined('AFFILIATE_CHECKALL') && !empty($referrer)) {
						...
						?>
						<p><?php _e('Alternativ kannst Du einfach direkt auf die unten stehende URL von der Seite verlinken, die Du in den erweiterten Einstellungen oben eingegeben hast:', 'affiliate'); ?></p>
				...
				?>
				<p><?php echo sprintf(__('Wenn Du lieber ein Banner oder eine Schaltfläche verwenden möchtest, haben wir <a href="%s">hier</a> eine große Auswahl an Größen.', 'affiliate') ,$bannerlink ); ?></p>
				<?php } ?>
				<p><?php _e('<strong>Du kannst Deine Empfehlungssummen überprüfen, indem Du die Details auf dieser Seite anzeigst</strong>', 'affiliate'); ?></p>
			<?php
			}

			echo '<p class="submit">';
			echo '<input type="submit" name="Submit" value="' . __('Einstellungen aktualisieren','affiliate') . '" /></p>';
