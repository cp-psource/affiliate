...
*/
	...
	function no_items() {
		_e( 'Es wurden keine passenden Benutzer gefunden.' );
	}
	...
	function show_filters_blog() {
		...
		if ((defined('SUBDOMAIN_INSTALL')) && (SUBDOMAIN_INSTALL == true)) {
			...
			$placeholder_text .= ', '. __('Domain Pfad', 'affiliate');
		}
		$placeholder_text .= __(' oder Mapped Domain', 'affiliate');
	...
	function get_columns() {
		...
		$columns['username'] 	= __( 'Benutzername', 'affiliate' );
		...
		if (aff_get_option('affiliateenableapproval', 'no') == 'yes') {
			$columns['approval'] = __('Genehmigt', 'affiliate');
		}
			
		$columns['paypal']    	= __( 'PayPal Account', 'affiliate' );
		$columns['reference']	= __( 'Referenz', 'affiliate' );
	...
	function single_row( $user_object, $style = '' ) {
		...
		if ( current_user_can( 'list_users' ) ) {
			...
			$actions['edit'] = '<a title="'. __('Partnerprogramm verwalten','affiliate') .'" href="' . $this->url_base .'&amp;subpage=summary&amp;id='. $user_object->ID .'" class="edit">'. __('verwalten','affiliate') . '</a>';
				...
				case 'blog': 
					...
					if (!empty($primary_blog)) {
						...
						if (($blog_details) && (!empty($blog_details))) {
							...
							$actions['affiliate-filter'] = '<a title="'. __('Filterliste nach diesem Blog', 'affiliate') .'" href="'. $filter_url  .'" >'. __('Filter', 'affiliate') .'</a>';
							$actions['affiliate-dashboard'] = '<a title="'. __('Besuche das Dashboard', 'affiliate') .'" href="'. get_admin_url($blog_details->blog_id) .'" >'. __('Dashboard', 'affiliate') .'</a>';
							$actions['affiliate-dashboard-admin'] = '<a title="'. __('Besuche das Dashboard', 'affiliate') .'" href="'. esc_url(add_query_arg('page', 'affiliatesadmin', get_admin_url($blog_details->blog_id))) .'" >'. __('affiliate', 'affiliate') .'</a>';
							
							
							$actions['affiliate-visit'] = '<a title="'. __('Webseite besuchen', 'affiliate') .'" href="'. $blog_details->siteurl .'" >'. __('Besuchen', 'affiliate') .'</a>';
